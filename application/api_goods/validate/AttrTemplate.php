...
{
    protected $rule = [
        ['id', 'require|checkValue:id', '属性模板ID不能为空'],
        ['name', 'require|max:255|checkValue:name', '属性模板名称不能为空|属性模板名称最大限255字符'],
        ...
        switch ($rule) {
            case 'name':
                if ($this->currentScene == 'create') {
                    $attr_template = model('attr_templates')->findOne(['name' => $value]);
                } elseif ($this->currentScene == 'update') {
                    $attr_template = model('attr_templates')->findOne(['name' => $value, 'id' => ['neq', $id]]);
                } else {
                    return '验证场景不存在';
                }
