...
class DrpOrders extends Rest
{
    ...
    public function lists(Request $request)
    {
        ...
        if (isset($map['order_status'])) {
            $result = Db::query('
            SELECT order_id,order_status,order_goods_id,user_id,source_user_id,level FROM wancll_user_commission_logs
            ...
            SELECT order_id,order_status,order_goods_id,user_id,source_user_id,level,expect_money,real_money FROM wancll_user_commission_logs
            WHERE user_id = :user_id AND order_status = :order_status AND data_state = 1
        ...
        // 订单统计金额
        // $total_except_money = model('user_commission_logs', 'drp_model')->getExceptMoney(['user_id' => $this->unique_id]);
    ...
    public function logs(Request $request)
    {
        ...
        $count = model('user_commission_logs', 'drp_model')->getAllCount($map);
        $lists = model('user_commission_logs', 'drp_model')->getAll($map, $this->page_num, $this->page_limit);
