...
class Backup
{
    ...
    {
        if (is_null($file)) {
            $this->file = ['name' => date('Ymd-His'), 'part' => WANCLL_ENV];
        } else {
            if (is_array($file) && !array_key_exists("name", $file) && !array_key_exists("part", $file)) {
                $this->file = $file[0];
            } else {
                $this->file = $file;
            }
    ...
    {
        $db = self::connect();
        $tables = get_config_file('db')['init_tables'];
        $error = [];
        ...
        if(empty($error)){
            return ['code' => 1, 'msg' => '初始化数据库全部执行成功', 'data' => []];
        }else{
            return ['code' => 0, 'msg' => '初始化数据库部分执行失败', 'data' => $error];
        }
    }
