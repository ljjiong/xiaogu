 ...
 *  【分销商分红提现模型】
 */
class BonusApplys extends BaseModel
{
    // 指定数据表
    protected $table = 'wancll_bonus_applys';

    //类型转换
    protected $type = [
        'id'                 =>  'integer',
        'user_id'            =>  'integer',
        'user_bonus_log_ids' =>  'json',
        'money'              =>  'float',
    ...
    protected static function init()
    {
        ...
        self::beforeInsert(function ($model) {
            $model->apply_no = make_order_no('B');

            $log_ids    = model('user_bonus_logs', 'bonus_model')->getColumn(['order_status' => 3, 'status' => 0, 'user_id' => $model->user_id], 'id');
            $real_money = model('user_bonus_logs', 'bonus_model')->getRealMoney(['order_status' => 3, 'status' => 0, 'user_id' => $model->user_id]);

            $model->user_bonus_log_ids = $log_ids;
            $model->money              = $real_money;
            $model->intro              = '申请分红提现';
            $model->desc               = '申请分红提现：' . $real_money . '￥，申请时间：' . date('Y-m-d H:i:s');
            $model->status             = 1;
        });
        ...
        self::afterInsert(function ($model) {
            $update_data = [];
            foreach ($model->user_bonus_log_ids as $key => $value) {
                $update_data[] = [
                    'id'             => $value,
                    'status'         => 1,
                    'bonus_apply_id' => $model->id,
                    'apply_no'       => $model->apply_no,
                ];
            }
            model('user_bonus_logs', 'bonus_model')->batchEdit($update_data);

            model('user_info')->updateBonus($model->user_id);
        });
    }
    ...
    public function detail($id, $page_num, $page_limit)
    {
        ...
        if ($result) {
            $sql_str      = '';
            $array_length = count($result['user_bonus_log_ids']) - 1;
            foreach ($result['user_bonus_log_ids'] as $key => $value) {
                if ($key == $array_length) {
                    $sql_str .= 'id = ' . $value;
                } else {
                    $sql_str .= 'id = ' . $value . ' OR ';
            ...
            $lists = Db::query('
            SELECT id,order_id,order_status,order_goods_id,user_id,source_user_id,bonus_type,region_type,status FROM wancll_user_bonus_logs
